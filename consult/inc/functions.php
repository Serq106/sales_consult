...
add_action('wp_ajax_freeCons', 'freeCons');
add_action('wp_ajax_nopriv_freeCons', 'freeCons');
...
function freeCons()
{
    ...
    $toEmail = "750260@mail.ru, info@dymohodminsk.by ,openzamkoff@yandex.ru";
    ...
    $headers = 'From: '. $_SERVER['HTTP_HOST'] .' <no-reply@dymohodminsk.by>' . "\r\n";
...
add_action( 'wp_ajax_reload_product', 'ajax_reload_products' ); // wp_ajax_{ЗНАЧЕНИЕ ПАРАМЕТРА ACTION!!}
add_action( 'wp_ajax_nopriv_reload_product', 'ajax_reload_products' );  // wp_ajax_nopriv_{ЗНАЧЕНИЕ ACTION!!}
// первый хук для авторизованных, второй для не авторизованных пользователей

function ajax_reload_products(){
    ...
    if($pipe == 'sendvich'){
        $args = array(                                      // Начинаем выводить все товары которые есть у выводимой категории
            'post_type' => 'products',                      // Определяем параметры
            'posts_per_page' => '100',
            'order' => 'ASC',
            'tax_query' => array(
                array(
        ...
        $args = array(                                      // Начинаем выводить все товары которые есть у выводимой категории
            ...
            'tax_query' => array(
                array(
                    ...
                    'field' => 'slug',
                    'operator' => 'and',
                    'terms' =>array( $pipe, $diameter, $mark, $thickness )
                )
            )
        );
    }
    ...
    foreach ($my_query->posts as $post) {
        $text .= '
            <tr class=' . $class . '>
                <td class="first">' . $post->post_title . '</td>
                <td>' . $post->post_content . ' BYN</td>
            </tr>
        ';
        if ($class == '') {
            $class = ' col';
